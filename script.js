...
function generateSession(){
  ...
  setTimeout(function(){
    ...
    if(dup)return;
    var now=new Date();
    var rec={name:'Student1',rollNo:'69',subject:session.subject,room:session.room||'',instr:session.instr||'',sessionId:_sid,date:now.toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'}),time:now.toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}),ts:Date.now()};
    records.push(rec);
